...
O método elbow plot para determinação do número de clusters do k-means não demonstrou sensibilidade à exclusão das variáveis, tendo resultado no mesmo número k=3 de clusters da análise principal (Figura 5).
...
O método de agrupamento hierárquico se mostrou sensível à exclusão das variáveis acessórias.
