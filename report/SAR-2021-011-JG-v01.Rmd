...
```{r fig2, fig.cap="**Figura 2** Silhouette plot do cluster hierárquico."}
# ![Age](../figures/silhouette.png)
knitr::include_graphics("../figures/silhouette.png", error = FALSE)
```

```{r silhuetas, include=FALSE}
...
Estipulando o número ótimo de clusters em k = 4, as silhuetas das observações variam entre `r format.float(min(hc4_sum$si.summary))` e `r format.float(max(hc4_sum$si.summary))` com média `r format.float(hc4_sum$si.summary["Mean"])` e mediana `r format.float(hc4_sum$si.summary["Median"])`.
Este número de clusters pode ser identificado cortando o dendrograma na altura h = 2.2 (Figura 3).
...
Estipulando o número de clusters em k = 2, as silhuetas das observações variam entre `r format.float(min(hc2_sum$si.summary))` e `r format.float(max(hc2_sum$si.summary))` com média `r format.float(hc2_sum$si.summary["Mean"])` e mediana `r format.float(hc2_sum$si.summary["Median"])`.

<!-- A tabela 2 mostra as características dos 4 clusters. -->
Os quatro clusters criados não foram suficientes para segregar os deputados evangélicos dos não evangélicos, mas em sua maioria ficaram distribuídos em dois grupos (clusters 1 e 2 na Tabela 2).
...
tab_cl4 %>%
  as_kable(
    caption = "**Tabela 2** Características dos clusters de deputados federais eleitos em 2018."
  )
```

## Avaliação de k=2 clusters
...
tab_cl2 %>%
  as_kable(
    caption = "**Tabela 3** Características dos clusters de deputados federais eleitos em 2018."
  )
```
